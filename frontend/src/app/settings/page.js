...
export default function SettingsPage() {
    ...
    const [pushTestTitle, setPushTestTitle] = useState("MARKHINS HUB Test Notification");
    const [pushTestMessage, setPushTestMessage] = useState("This is a test notification from MARKHINS HUB Admin Console.");
    ...
    async function handleSavePushSetting(enabledVal, timeVal) {
        setPushBusy(true);
        setMsg("");
        setError("");
        try {
            const res = await updatePushSetting({ enabled: enabledVal, reminder_time: timeVal });
            setPushEnabled(enabledVal);
            setPushReminderTime(timeVal);
            setMsg("Web push notification settings updated successfully.");
            playSound('success');
        } catch (err) {
            playSound('error');
            setError(err.message);
    ...
    async function handleSendTestPush() {
        ...
        setError("");
        try {
            const res = await sendTestPushNotification(pushTestTitle, pushTestMessage);
            setMsg(res.message || `Test notification sent! Delivered: ${res.delivered || 0}, Failed: ${res.failed || 0}`);
            playSound('success');
        } catch (err) {
            playSound('error');
            setError(err.message);
                ...
                {activeTab === "system" && (
                    <div className="space-y-6">
                        ...
                        <div className="rounded-3xl border border-purple-100 bg-white p-6 shadow-sm">
                            ...
                            {/* Send Test Push Section */}
                            <div className="mt-6 pt-4 border-t border-gray-100">
                                <h3 className="text-xs font-black uppercase text-gray-700 tracking-wider mb-2">Test Push Broadcast</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                    <input
                                        type="text"
                                        value={pushTestTitle}
